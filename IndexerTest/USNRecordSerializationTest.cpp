...
namespace indexer {
    ...
    TEST_F(USNRecordSerializationTest, DeserializedRecordEqToOrig) {
		...
		const std::wstring serialized = ntfs_reader::helpers::SerializeRecord(*record_, drive_letter_);

		auto deserializedPair = ntfs_reader::helpers::DeserializeRecord(serialized);

        EXPECT_TRUE(deserializedPair.first != nullptr);
        EXPECT_EQ(deserializedPair.second, drive_letter_);

        auto record = std::move(deserializedPair.first);
